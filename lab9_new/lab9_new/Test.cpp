...
void Test::testRepository2() {
    ...
    Repository2 repoo(1);
    try {
        repoo.addMedicament(new_medicament);
        assert(false);
    }
    catch (exception&) {
        assert(true);
...
void Test::testRepositoryFile() {
    ofstream fout("test_file_repo.txt");
    fout << "";
    fout.close();

    RepoFile repooo("test_file_repo.txt");
    Medicament new_medicament(denumire, producator, substActiva, pret);
    Medicament other_medicament(other_denumire, other_producator, other_substActiva, other_pret);
    for (auto x : repooo.getMedicamente()) {
        cout << x.getDenumire() << endl;
    }
    /// Test add
    repooo.addMedicament(new_medicament);
    assert(repooo.getLen() == 1);
    try {
        repooo.addMedicament(new_medicament);
        assert(false);
    }
    ...
    repooo.addMedicament(other_medicament);
    assert(repooo.getLen() == 2);

    /// Test delete
    //auto medicament_position = repo.findMedicament(denumire);
    //assert(medicament_position!=repo.getMedicamente().end());
    repooo.deleteMedicament(denumire);
    assert(repooo.getLen() == 1);
    try {
        repooo.deleteMedicament(denumire);
        assert(false);
    ...
    repooo.updateMedicament(other_denumire, new_medicament);
    assert(repooo.getLen() == 1);

    try {
        repooo.updateMedicament("ROSPEITJJFODPR", other_medicament);
        assert(false);
    }
    catch (exception&) {
        assert(true);
    ...
    auto all = repooo.getMedicamente();
    assert(all[0].getDenumire() == denumire);
    assert(all[0].getProducator() == producator);
    assert(all[0].getSubstActiva() == substActiva);
    assert(all[0].getPret() == pret);
    ...
    RepoFile repoo2("test_file_repo2.txt");
    cout << "RepoFile tests passed"<<endl;
...
void Test::testServiceforRepo2() {
    ...
    service.exportToFile("indexx.html");
    service.exportToFile("testare.cvs");
